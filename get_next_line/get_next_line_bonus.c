...
/*   Updated: 2024/01/25 18:06:06 by aggrigor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE < 1 || fd < 0 || read(fd, result, 0) == -1)
		return (NULL);
	if (files[fd] == NULL)
		files[fd] = ft_lstnew(NULL);
	if (!ft_read_line(fd, files[fd]) || files[fd]->value == NULL)
	{
		free_prev_parts(files + fd, free);
		return (NULL);
	}
	line_len = count_line_len(files[fd]);
	result = (char *)malloc(line_len + 1);
	if (result == NULL)
		return (NULL);
	result = make_result_line(result, files + fd, line_len);
	return (result);
}
